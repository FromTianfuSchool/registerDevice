...
class Upload:
    def __init__(self):
        ...
        self._cookies = self.read_cookies(utils.cookiesPath)
    ...
    def upload(self, dev_info="name", print_fuc=print):
        """
        @param dev_info: data of device information
        """
        data_table = self.loadTemplate(utils.samplePath)
        # load_data = self.generatePeriodData()
        load_data = self.loadDataExcel()
    ...
    def loadDataExcel(self):
        ...
        """
        wb = load_workbook(utils.deviceinfoPath, data_only=True)
        ...
        for row in rows_obj[3:]:
            if row[0].value:  # 空白行为数据录入结束标志位
                ...
                ddict = dict(zip(name_list, value_list))
                ddict['data[start_time]'] = ddict[
                    'data[start_time]'] + ' ' + ddict['data[start_hour]']
                data_dict.append(ddict)
        return data_dict
        # 发动机排气采样分析测试系统 ZPJ190 发动机排放检测部 2019-09-06 00:00:00 8 None 蒋鑫 None 13368385052 蒋鑫 车用压燃式发动机排气污染物 None

    # loadDataExcel()

    def generatePeriodData(self, period=1):
        '''
        ...
        if allrows[3][0].value:
            ...
            group = dict(zip(d_keys, d_values))
            dates = self.getperiod(group['data[start_time]'])
            for date in dates:
                group['data[start_time]'] = date + \
                    ' ' + group['data[start_hour]']
                d_dict.append(group)
            return d_dict

    def getperiod(self, startdate):
        '''
